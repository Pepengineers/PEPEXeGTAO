...
namespace PEPEngineers.ThirdParty
{
    [SuppressMessage("ReSharper", "InconsistentNaming")]
    internal static class ThirdPartyXeGTAO
    {
        ...
        {
            var index = 0U;
            for (var curLevel = XE_HILBERT_WIDTH / 2U; curLevel > 0U; curLevel /= 2U)
            {
                var regionX = (posX & curLevel) > 0U ? 1U : 0U;
                var regionY = (posY & curLevel) > 0U ? 1U : 0U;
        ...
        public struct XeGTAOConstantsCS
        {
        ...
        public struct XeGTAOSettings
        {
            ...
            public static XeGTAOSettings Default => new()
            {
                QualityLevel = 2,
                DenoisePasses = 1,
                Radius = 0.5f,
            ...
            public static unsafe void UpdateConstants(ref XeGTAOConstantsCS consts, int viewportWidth,
                int viewportHeight, in XeGTAOSettings settings,
                float4x4 projMatrix,
                bool rowMajor, uint frameCounter)
            {
                var pProjMatrix = (float*)UnsafeUtility.AddressOf(ref projMatrix);
                ...
                var depthLinearizeMul =
                    rowMajor
                        ? -pProjMatrix[3 * 4 + 2]
                        : -pProjMatrix[
                            3 + 2 * 4]; // float depthLinearizeMul = ( clipFar * clipNear ) / ( clipFar - clipNear );
                var depthLinearizeAdd =
                    rowMajor
                        ? pProjMatrix[2 * 4 + 2]
                        : pProjMatrix[2 + 2 * 4]; // float depthLinearizeAdd = clipFar / ( clipFar - clipNear );
                ...
                var tanHalfFOVY =
                    1.0f / (rowMajor
                        ? pProjMatrix[1 * 4 + 1]
                        : pProjMatrix[1 + 1 * 4]); // = tanf( drawContext.Camera.GetYFOV( ) * 0.5f );
                var tanHalfFOVX =
                    1.0F / (rowMajor
                        ? pProjMatrix[0 * 4 + 0]
                        : pProjMatrix[0 + 0 * 4]); // = tanHalfFOVY * drawContext.Camera.GetAspect( );
